...
import (
	...
	"github.com/Photon-Panel/Photon-Daemon/server/backup"
)
	...
	if err := s.client.SetBackupStatus(s.Context(), uuid, ad.ToRequest(successful)); err != nil {
		if !remote.IsRequestError(err) {
			s.Log().WithFields(log.Fields{
				"backup": uuid,
				"error":  err,
			}).Error("failed to notify panel of backup status due to daemon error")
			return err
		}
...
// Get all of the ignored files for a server based on its .photonignore file in the root.
func (s *Server) getServerwideIgnoredFiles() (string, error) {
	f, st, err := s.Filesystem().File(".photonignore")
	if err != nil {
