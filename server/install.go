...
func (ip *InstallationProcess) AfterExecute(containerId string) error {
	...
	tmpl, err := template.New("header").Parse(`Photon Panel Installation Log
...
func (ip *InstallationProcess) Execute() (string, error) {
	...
	// not exist when this runs if daemon boots with a missing directory and a user
	...
	if "macvlan" == serverNetConfig.Driver { //Generate networking config for macvlan driver
		...
		netConf = &network.NetworkingConfig{
			EndpointsConfig: map[string]*network.EndpointSettings{
				serverNetConfig.Name: { //Get network name from daemon config
					IPAMConfig: &network.EndpointIPAMConfig{
						IPv4Address: defaultMapping.Ip,
					},
					IPAddress: defaultMapping.Ip, //Use default mapping ip address (the daemon supports only one network per server)
