...
func RequireAuthorization() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Header("User-Agent", fmt.Sprintf("Photon Daemon/v%s (id:%s)", system.Version, config.Get().AuthenticationTokenId))
		...
		// All requests to the daemon must be authorized with the authentication token present in
		// the daemon configuration file. Remeber, all requests to the daemon come from the Panel
