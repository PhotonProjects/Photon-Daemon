...
var configureCmd = &cobra.Command{
	Use:   "configure",
	Short: "Use a token to configure the daemon automatically",
	Run:   configureCmdRun,
}
...
func configureCmdRun(cmd *cobra.Command, args []string) {
	...
	fmt.Println("Successfully configured the daemon.")
}
...
func validateField(name string, str string) error {
	...
	case "token":
		if !regexp.MustCompile(`^(phot|papp)_(\w{43})$`).Match([]byte(str)) {
