...
import (
	...
	"github.com/Photon-Panel/Photon-Daemon/server/filesystem/quotas"
	"github.com/spf13/cobra"
	"golang.org/x/crypto/acme"
	"golang.org/x/crypto/acme/autocert"
	"golang.org/x/sys/unix"

	"github.com/Photon-Panel/Photon-Daemon/config"
	"github.com/Photon-Panel/Photon-Daemon/environment"
	"github.com/Photon-Panel/Photon-Daemon/internal/cron"
	"github.com/Photon-Panel/Photon-Daemon/internal/database"
	"github.com/Photon-Panel/Photon-Daemon/loggers/cli"
	"github.com/Photon-Panel/Photon-Daemon/remote"
	"github.com/Photon-Panel/Photon-Daemon/router"
	"github.com/Photon-Panel/Photon-Daemon/server"
	"github.com/Photon-Panel/Photon-Daemon/sftp"
...
var rootCommand = &cobra.Command{
	Use:   "photon-daemon",
	Short: "Runs the API server allowing programmatic control of game servers for Photon Panel.",
	PreRun: func(cmd *cobra.Command, args []string) {
		initConfig()
		initLogging()
		if tls, _ := cmd.Flags().GetBool("auto-tls"); tls {
			if host, _ := cmd.Flags().GetString("tls-hostname"); host == "" {
				fmt.Println("A TLS hostname must be provided when running the daemon with automatic TLS, e.g.:\n\n    ./photon-daemon --auto-tls --tls-hostname my.example.com")
				os.Exit(1)
			}
		}
	},
...
var versionCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		fmt.Printf("photon-daemon v%s\nCopyright © 2026 - %d M4xyll, Jamesfrench & Contributors\n", system.Version, time.Now().Year())
	},
}
...
func init() {
	...
	rootCommand.PersistentFlags().BoolVar(&debug, "debug", false, "pass in order to run the daemon in debug mode")
	...
	rootCommand.Flags().Bool("auto-tls", false, "pass in order to have the daemon generate and manage its own SSL certificates using Let's Encrypt")
	rootCommand.Flags().String("tls-hostname", "", "required with --auto-tls, the FQDN for the generated SSL certificate")
	rootCommand.Flags().Bool("ignore-certificate-errors", false, "ignore certificate verification errors when executing API calls")
	rootCommand.Flags().Bool("standalone", false, "run without a panel connection (standalone mode)")
...
func rootCmdRun(cmd *cobra.Command, _ []string) {
	...
	log.WithField("timezone", config.Get().System.Timezone).Info("configured the daemon with system timezone")
	if err := config.ConfigureDirectories(); err != nil {
		log.WithField("error", err).Fatal("failed to configure system directories for photon")
		return
	}
	if err := config.EnsurePhotonUser(); err != nil {
		log.WithField("error", err).Fatal("failed to create photon system user")
		return
	}
	if err := config.ConfigurePasswd(); err != nil {
		log.WithField("error", err).Fatal("failed to create passwd files for photon")
	}
	...
	var err error
	if standalone {
		log.Warn("running in standalone mode — no panel connection")
	...
	for _, serv := range manager.All() {
		...
		pool.Submit(func() {
			...
			// end up waiting a long period of time for all the images to be re-pulled on daemon boot rather
	...
	go func() {
		...
		// currently marked as installing/restoring now that the daemon is restarted.
...
func initLogging() {
	...
	p := filepath.Join(dir, "/photon-daemon.log")
	w, err := logrotate.NewFile(p)
	if err != nil {
		log2.Fatalf("cmd/root: failed to create photon-daemon log: %s", err)
	}
...
// Prints the photon daemon logo, nothing special here!
func printLogo() {
	fmt.Printf(colorstring.Color(`
[blue][bold]  ⚡ Photon Daemon[reset]  [green]v%s[reset]
...
The daemon was not able to locate your configuration file, and therefore is not
able to complete its boot process. Please ensure you have copied your instance
configuration file into the default location below.

Default Location: /etc/photon/config.yml
