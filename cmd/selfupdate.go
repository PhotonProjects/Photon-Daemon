...
func newSelfupdateCommand() *cobra.Command {
	command := &cobra.Command{
		Use:   "update",
		Short: "Update the daemon to the latest version",
		Run:   selfupdateCmdRun,
	}

	command.Flags().StringVar(&updateArgs.repoOwner, "repo-owner", "Photon-Panel", "GitHub repository owner")
	command.Flags().StringVar(&updateArgs.repoName, "repo-name", "photon-daemon", "GitHub repository name")
...
func selfupdateCmdRun(_ *cobra.Command, _ []string) {
	...
	fmt.Println("\nUpdate successful! Please restart the photon daemon service (e.g., systemctl restart photon-daemon)")
}
...
func performUpdate(version, binaryName string) error {
	...
	tmpDir, err := os.MkdirTemp("", "photon-update-*")
	if err != nil {
