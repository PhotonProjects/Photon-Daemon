...
import (
	...
	"github.com/Photon-Panel/Photon-Daemon/system"
)

const DefaultLocation = "/etc/photon/config.yml"
...
// exposed by the Photon Daemon webserver.
...
// from the daemon to the Panel.
type RemoteQueryConfiguration struct {
	// The amount of time in seconds that the daemon should allow for a request to the Panel API
	...
	// Photon Daemon instance. A single request is initially made to the Panel to get this number
	...
	//The mentioned headers can be appended to queries sent from the daemon to the panel.
	CustomHeaders map[string]string `yaml:"custom_headers"`
}

// SystemConfiguration defines basic system configuration settings.
type SystemConfiguration struct {
	// The root directory where all of photon data is stored at.
	RootDirectory string `default:"/var/lib/photon" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other daemon events are logged.
	LogDirectory string `default:"/var/log/photon" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/photon/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/photon/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/photon/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/photon" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"photon" yaml:"username"`

	// The timezone for this Photon Daemon instance. This is detected by the daemon automatically if possible,
	// and falls back to UTC if not able to be detected. If you need to set this manually, that
	// can also be done.
	//
	// This timezone value is passed into all containers created by the daemon.
	...
	User struct {
		...
		Passwd struct {
			...
			Directory string `json:"directory" yaml:"directory" default:"/etc/photon"`
	...
	MachineID struct {
		...
		Directory string `json:"directory" yaml:"directory" default:"/etc/photon/machine-id"`
...
type CrashDetection struct {
	...
	// Determines if the daemon should detect a server that stops with a normal exit code of
	// "0" as being crashed if the process stopped without any daemon interaction. E.g.
...
type Configuration struct {
	...
	// Determines if the daemon should be running in debug mode. This value is ignored
	// if the debug flag is passed through the command line arguments.
	Debug bool

	// Determines if the daemon should run with minimal logging output. This value is
	// ignored if debug mode is enabled.
	Quiet bool

	AppName string `default:"photon" json:"app_name" yaml:"app_name"`
	...
	// is only required by users running the daemon without SSL certificates and using internal IP
...
// EnsurePhotonUser ensures that the Photon core user exists on the
...
func EnsurePhotonUser() error {
	sysName, err := getSystemName()
	if err != nil {
		return err
	}

	// Our way of detecting if the daemon is running inside of Docker.
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("PHOTON_USERNAME"), "photon")
		_config.System.User.Uid = system.MustInt(system.FirstNotEmpty(os.Getenv("PHOTON_UID"), "988"))
		_config.System.User.Gid = system.MustInt(system.FirstNotEmpty(os.Getenv("PHOTON_GID"), "988"))
		return nil
	}
	...
	log.WithField("username", _config.System.Username).Info("checking for photon system user")
...
func FromFile(path string) error {
	...
	c.Token = Token{
		ID:    os.Getenv("PHOTON_TOKEN_ID"),
		Token: os.Getenv("PHOTON_TOKEN"),
	}
	if c.Token.ID == "" {
...
// this looks cleaner than the previous way and is similar to the original
...
// EnableLogRotation writes a logrotate file for the daemon to the system logrotate
...
func EnableLogRotation() error {
	if !_config.System.EnableLogRotate {
		log.Info("skipping log rotate configuration, disabled in daemon config file")
		return nil
	}
	...
	if _, err := os.Stat("/etc/logrotate.d/photon-daemon"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	// but there is not a file for the daemon already. In that case, let us write a new file to
	// it so files can be rotated easily.
	f, err := os.Create("/etc/logrotate.d/photon-daemon")
	if err != nil {
		return err
	}
	defer f.Close()

	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/photon-daemon.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP photon-daemon.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
...
func ConfigureTimezone() error {
	...
	if _config.System.Timezone == "" {
		...
		if err != nil {
			...
			// fall through to UTC to get daemon booted at least.
